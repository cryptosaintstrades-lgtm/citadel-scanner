...
# v32.2 institutional quality engine + visible HTF confirmation settings
...
S_TIER_REQUIRES_4H = False  # Balanced mode: 4H adds quality/bonus but is not required for A+ alerts
...
def structure_component_score(coin, side):
    score = 0
    if side == "SHORT":
...
def htf_component_score(coin, side):
    # Balanced mode: 1H is the main confirmation, 4H is a bonus.
    score = 0
    if htf_structure_agrees(coin, "1h"):
        score += 14
    if htf_structure_agrees(coin, "4h"):
...
def volume_component_score(coin):
    score = 0
    s5 = safe_float(coin.get("volume_spike_5m"))
    s15 = safe_float(coin.get("volume_spike_15m"))
...
def liquidity_component_score(coin, side):
    score = 0
    move = abs(safe_float(coin.get("change_24h")))

    if side == "SHORT":
        if coin.get("sweep_high_5m"):
            score += 10
        if safe_float(coin.get("pullback_from_high")) >= 3:
            score += 5
        if move >= 20:
            score += 5
        elif move >= 10:
        ...
        if coin.get("sweep_low_5m"):
            score += 10
        if safe_float(coin.get("bounce_from_low")) >= 3:
            score += 5
        if move >= 20:
            score += 5
        elif move >= 10:
...
def location_component_score(coin, side):
    score = 0
    rp = safe_float(coin.get("range_pos"))
...
def institutional_quality_breakdown(coin, side):
    structure = structure_component_score(coin, side)
    htf = htf_component_score(coin, side)
    volume = volume_component_score(coin)
    liquidity = liquidity_component_score(coin, side)
    location = location_component_score(coin, side)
    ...
    return {
        ...
        "one_h_aligned": htf_structure_agrees(coin, "1h"),
        "four_h_aligned": htf_structure_agrees(coin, "4h"),
    }


def apply_quality_engine(result):
    side = result.get("trade_bias", "")
    if side not in ["SHORT", "LONG"]:
        result["quality_total"] = 0
        result["quality_grade"] = ""
        return result

    q = institutional_quality_breakdown(result, side)
    result["quality_structure"] = q["structure"]
    result["quality_htf"] = q["htf"]
    result["quality_volume"] = q["volume"]
    result["quality_liquidity"] = q["liquidity"]
    result["quality_location"] = q["location"]
    result["quality_total"] = q["total"]
    result["quality_grade"] = q["grade"]
    result["htf_alignment"] = q["htf_alignment"]
    result["one_h_aligned"] = q["one_h_aligned"]
    result["four_h_aligned"] = q["four_h_aligned"]

    # Rebalance the active side score/grade using the institutional score.
    if side == "SHORT":
        result["short_score"] = q["total"]
        result["short_grade"] = q["grade"]
        result["signal"] = f"{q['grade']} SHORT WATCH" if q["grade"] else "Neutral"
    elif side == "LONG":
        result["long_score"] = q["total"]
        result["long_grade"] = q["grade"]
        result["signal"] = f"{q['grade']} LONG WATCH" if q["grade"] else "Neutral"

    return result


def format_quality_breakdown(coin):
    return (
        ...
        f"Location: {coin.get('quality_location', 0)}/20 | "
    ...
    embed = {
        ...
        "fields": [
            ...
            },
            {
                ...
                "value": (
                    f"1H: **{coin.get('structure_note_1h', 'Not checked')}** {quality_status(coin.get('one_h_aligned', False))}\n"
                    f"4H: **{coin.get('structure_note_4h', 'Not checked')}** {quality_status(coin.get('four_h_aligned', False))}\n"
...
def print_performance_dashboard():
    ...
    print("\n" + "=" * 80)
    print("SCANNER PERFORMANCE DASHBOARD v32.2")
    print("=" * 80)
...
def print_leaderboard_dashboard():
    ...
    print("\n" + "=" * 80)
    print(f"SCANNER LEADERBOARD v32.2 — LAST {LEADERBOARD_LOOKBACK_HOURS} HOURS")
    print("=" * 80)
...
def build_results(tickers):
    ...
    for m in base:
        ...
        result = apply_quality_engine(result)
...
def print_section(title, coins, sort_key, limit=10):
    ...
    for coin in coins[:limit]:
        ...
        print(
            f"HTF CONFIRMATION | 1H: {coin.get('structure_note_1h', 'Not checked')} {quality_status(coin.get('one_h_aligned', False))} | "
            f"4H: {coin.get('structure_note_4h', 'Not checked')} {quality_status(coin.get('four_h_aligned', False))} | "
            f"Alignment: {coin.get('htf_alignment', 0)}/2"
        )
        print(f"QUALITY BREAKDOWN | {format_quality_breakdown(coin)}")
        print_reasons(coin)
...
def run_scanner():
    print("\n" + "=" * 190)
    print("CITADEL TRADE OPPORTUNITY SCANNER v32.2 — INSTITUTIONAL QUALITY + VISIBLE HTF")
    ...
    print("Reminder: v32.2 uses balanced HTF confirmation, quality score breakdowns, and visible 1H/4H diagnostics.")
