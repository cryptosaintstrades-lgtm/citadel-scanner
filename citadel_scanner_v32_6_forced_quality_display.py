...
def print_quality_breakdown(coin):
    coin = ensure_quality_fields(coin)
    # v32.6: one-line summary makes Railway search filters useful.
...
def should_send_alert(coin):
    ...
    if ELITE_ONLY:
        if grade_value not in ["S", "A+"]:
            return False
        if not structure_agrees(coin):
            return False
        if coin.get("htf_alignment", 0) < ELITE_MIN_HTF_ALIGNMENT:
            return False
    else:
        if grade_value not in ["A", "A+", "S"]:
            return False
...
def run_scanner():
    print("\n" + "=" * 190)
    print("CITADEL TRADE OPPORTUNITY SCANNER v32.4 — CLEAN HTF DISCORD ENGINE")
    ...
    print("Reminder: v32.4 uses clean Discord HTF display + balanced HTF elite alert filter.")
